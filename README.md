...
```sh
nix run github:abs/mysql-mcp -- version
nix profile install github:abs/mysql-mcp
```

With Go:

```sh
go install github.com/abs/mysql-mcp/cmd/mysql-mcp@latest
```
...
```jsonc
{
  "$schema": "https://github.com/abs/mysql-mcp/raw/main/schema/config.schema.json",
  "sources": {
    "local": {
      "engine": "mysql",
      "host": "127.0.0.1",
...
```sh
nix develop          # go, gopls, golangci-lint, gomarkdoc
go test ./...
go build ./...
```

Regenerate derived artifacts (CI also does this automatically):

```sh
go run ./cmd/mysql-mcp gen-schema          # schema/config.schema.json
go run ./cmd/mysql-mcp gen-config-docs     # docs/configuration.md
gomarkdoc --output docs/api.md ./...       # docs/api.md
bash scripts/update-vendor-hash.sh         # flake.nix vendorHash
```
